...
from owlsight.configurations.schema
...
class GenerationRequest(BaseModel):
    ...
    temperature: float = Field(default=0.7, description="Generation temperature (0.0 = deterministic)")
    stopwords: Optional[List[str]] = Field(default=None, description="List of words to stop generation at")
    stream: bool = Field(default=False, description="Whether to stream the response")
    generation_kwargs: Optional[Dict[str, Any]] = Field(default=None, description="Additional generation parameters")
...
async def generate(
    model_key: str,
    request: GenerationRequest,
    model: TextGenerationProcessor = Depends(get_model)
