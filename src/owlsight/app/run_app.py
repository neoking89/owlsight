    ...
    while True:
        try:
            ...
            if manager.processor is None:
                logger.error("Processor not set! Please load a model first by setting by either:") 
                logger.error("1: Setting 'model.model_id' in the config:model section")
                logger.error("2: Loading an existing configuration with the 'load' command")
                logger.error("3: Search and select a model through the Huggingface model hub in the model:huggingface section")
                continue
            else:
